...
class Embedding(nn.Module):
    def __init__(self, configs):
        ...
        self.patch_embeddings = Conv3d(in_channels=configs['emb_in_channels'],
...
class MLP(nn.Module):
    def __init__(self, hidden_dim, mlp_dim, dp_rate):
        super(MLP, self).__init__()
        self.fc1 = Linear(hidden_dim, mlp_dim)
        self.fc2 = Linear(mlp_dim, hidden_dim)
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding=1):
        ...
        self.block = nn.Sequential(
            Conv3d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, padding=padding),
...
class ConvLayer(nn.Module):
    def __init__(self, conv_num, in_channels, out_channels, kernel_size):
        ...
        self.layer = nn.ModuleList()
        self.block0 = ConvBlock(in_channels, out_channels, 1, padding=0)
...
class Encoder(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_l1 = ConvLayer(2, 1, 64, 3)
        self.d1 = MaxPool3d(kernel_size=2, stride=2)
        self.conv_l2 = ConvLayer(2, 64, 128, 3)
        self.d2 = MaxPool3d(kernel_size=2, stride=2)
        self.conv_l3 = ConvLayer(2, 128, 256, 3)
...
class CUP(nn.Module):
    def __init__(self, configs):
        ...
        decoder_blocks = [DecoderBlock(in_ch, out_ch, skip_ch) for in_ch, out_ch, skip_ch in
                          configs['decoder_channels']]
...
if __name__ == '__main__':
    img = torch.randn(1, 1,192,192,192)# 256, 256, 256)
    configs = {
        'hidden_dim': 768,
        'input_size': (48, 48, 48),
        'patch_size': (4, 4, 4),
